// Given the following csv data, select a single price for every store,
// UPC combination. The price selected should be based on the day of the
...
function readAllCSVData() {
    return `date,host,store_id,postal_code,upc,price
  ...
  function dataArrayofObjects(){
    const csvArray = splitData()
    const arrayData = []
    
    for(let i = 6; i < csvArray.length; i += 6){
      const data = {}
      data['date'] = csvArray[i]
      data['store_id'] = csvArray[i+2]
      data['upc'] = csvArray[i+4]
      data['price'] = csvArray[i+5]
  ...
  // go through array, where store_id and upc are the same then create nested object of data and price, if different then create new object 
  
  function nestData(){
    const dataArray = dataArrayofObjects()
    
    const storeObject = {}
    storeObject['store_id'] = ''
    storeObject['upc'] = ''
    storeObject['details'] = {}
    storeObject['details']['price'] =[]
    storeObject['details']['date'] =''

    
    dataArray.forEach(row => {
       
        storeObject['store_id'] = row['store_id']
        storeObject['upc'] = row['upc']
  
    if(row['store_id'] === storeObject['store_id'] && row['upc'] === storeObject['upc'] || !storeObject['store_id']){
            if(storeObject['details']['date'] !== row['date']){
                const date = new Date (row['date'])
                storeObject['details']['date'] = date.getDay()
                storeObject['details']['price'].push(row['price'])
            } else {
                storeObject['details']['price'].push(row['price'])
            }
     } else {
        ...
        storeObject['details'] = {}
        storeObject['details']['price'] = []
        storeObject['details']['date'] = row['date']
        storeObject['details']['price'].push(row['price'])
      }
